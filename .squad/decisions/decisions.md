...
## 2026-04-20T01:29:00Z: Adopt Zustand for Client State Management (Elminster & Regis)

**By:** Elminster (Architect) + Regis (Frontend Dev)  
**Date:** 2026-04-20  
**Status:** ✅ RECOMMENDED — Ready for Implementation Sprint  
**Scope:** Client-side state management refactor (4-phase incremental migration)

### Context

The client currently uses **React Context + useReducer** pattern (monolithic 30-field `AppState`). This works but shows architectural strain under real-time game load:

- **Re-render blast radius (HIGH):** Every component calling `useAppContext()` re-renders on ANY state change. During combat, WebSocket delivers 10-50 state updates/sec — each triggers full component tree re-renders regardless of whether a component uses that state.
- **Monolithic state blob (MEDIUM):** 30 fields spanning auth, connection, combat, inventory, equipment, occupants, stats, chat, and overlay jammed into one object. Single reducer switch/case handles everything.
- **Large WebSocket handler hook (HIGH):** `useZoneConnection.ts` is 577 lines with 15 inline message handlers (~300 lines), each dispatching 1-4 actions. File grows linearly with every new server message type. Riskiest refactor target.
- **Duplicated handler registration (MEDIUM):** `connection.ts` `connect()` and `switchRoom()` have identical 35-line handler registration blocks. Every new message type requires updates in 4 places.
- **Test boilerplate (MEDIUM):** 9 test files manually create identical `useReducer` + `AppContext.Provider` wrappers (~15 lines each).

### Decision

**Adopt Zustand** as the new client state management library.

**Why Zustand:**

| Pain Point | Zustand Solution |
|------------|-----------------|
| Re-render blast | **Selectors:** `useStore(s => s.playerHp)` re-renders ONLY when `playerHp` changes. Single biggest win for real-time game. |
| Monolithic state | **Store slices:** `createAuthSlice`, `createCombatSlice`, `createInventorySlice`, etc. Each slice owns its state + actions. Better code organization. |
| WebSocket handlers | **Direct mutations + outside React:** `set({ playerHp: msg.hp })` vs action objects. Handlers call store methods directly, no dispatch indirection, no closure staling. |
| Handler registration duplication | **Eliminated:** Store methods accessible outside React — `connection.ts` calls `getStore().setCombatState()` directly. |
| Test boilerplate | **Simplified:** Zustand stores are module singletons. Create fresh store per test, no provider wrapping. Simpler mock pattern. |

**Excluded alternatives:**
- **Redux Toolkit:** Too heavyweight for game client
- **Jotai:** Atomic model doesn't match domain-slice structure
- **Valtio:** Proxy-based harder to reason about in message-driven architecture

### Implementation: 4-Phase Incremental Migration

**Total effort:** 3.5-4.5 days focused work. Each phase is reviewable independently. Rollback possible at any point.

#### Phase 1 — Parallel Store (1 day)
- Create Zustand store alongside existing Context
- Both coexist; no removal yet
- New features use Zustand
- Existing code untouched
- **Risk:** Low (additive, no removal)

#### Phase 2 — Migrate WebSocket Handlers (1 day)
- Move `useZoneConnection.ts` message handlers to call Zustand store directly
- This is the **highest-value change** — eliminates re-render blast from combat messages
- **Risk:** Medium (largest refactor, most complex file, must be careful with testing)
- **Reward:** Biggest performance win

#### Phase 3 — Migrate Components (1-2 days)
- Replace `useAppContext()` calls with Zustand selectors
- One component/file at a time
- Each change is a small, reviewable PR
- ~23 component/hook files affected
- **Risk:** Low (mechanical changes)

#### Phase 4 — Remove Context (0.5 day)
- Delete `AppContext`, `appReducer`, `useAppContext` hook
- Remove `<AppContext.Provider>` from `App.tsx`
- Update remaining tests that directly import store
...
|----------|-------|--------|
| Store definition | 1 (store.ts → Zustand setup) | Medium |
| Main app (App.tsx) | 1 (remove Provider, localStorage sync) | Small |
| Hooks (useZoneConnection, useFlags, useSettings, useWhoList, etc.) | ~8 | Medium |
| Components (StatusPanel, CombatHUD, CombinedStashLoadout, etc.) | ~12 | Small each |
| Pages (Login, CharacterSelect, Settings, ZoneExploration, etc.) | ~6 | Small each |
| Services (connection.ts) | 1 | Medium |
| Tests | ~10-15 affected | Medium total |
| **Total** | **~30-40 files** | **3.5-4.5 days** |

### Conditions for Success

1. **Incremental only.** No big-bang rewrite. Follow the 4 phases in order.
2. **Move Colyseus Room out of store.** It's already in `roomRef` in `useZoneConnection` — the duplication in `AppState.room` is unnecessary. Room is a live SDK object; it belongs in a ref, not reactive state.
3. **Domain slices from day one.** Split into auth, combat, inventory, connection, terminal slices. Don't recreate a monolithic store.
4. **Keep connection.ts service layer clean.** Zustand store methods replace the `MessageHandlers` interface, but the service stays framework-agnostic.

### Zustand Features to Use

- **`create()` with slices:** Domain separation
- **Selector pattern with shallow equality:** `useStore(state => ({ inCombat, messages }), shallow)` for multi-field selects
- **`immer` middleware:** For complex nested updates (equipment slots, room occupants) — future-proofing
- **`subscribeWithSelector`:** WebSocket handlers subscribe to specific state changes without rendering
- **`devtools` middleware:** Free Redux DevTools integration for debugging real-time state

### Risks & Mitigations

| Risk | Severity | Mitigation |
|------|----------|-----------|
| Migration disruption (~30 files) | MEDIUM | Incremental phases. Each phase independently testable. |
| WebSocket handler re-wiring | MEDIUM | Phase 2 is scoped narrowly. Heavy testing required. Pair-review recommended. |
| Test rewrite (28 test files) | LOW-MEDIUM | Zustand tests are simpler than Context mocking. Pattern is more straightforward. |
| Half-migrated state confusion | MEDIUM | Document slice ownership clearly. Use phase gates (e.g., "Phase 2 complete" before Phase 3 starts). |

### No Breaking Changes

- Zustand stores are drop-in replacements for Context
- WebSocket handlers are internal refactor (no API change)
- Component selectors are simpler than current `useAppContext()` + destructuring
- All existing tests can be rewritten with equivalent or better clarity

### Next Steps

1. **Sprint planning:** Create 4 separate backlog items (Phase 1-4) with dependencies
2. **Phase 1 prep:** Review Zustand docs, sketch store slices (auth, combat, inventory, connection, terminal)
3. **High-risk review:** Identify testing strategy for Phase 2 (WebSocket handlers)
4. **Team capacity:** Assign primary developer (recommend backend-experienced to handle store logic clearly)
5. **Rollout:** Begin Phase 1 after next sprint planning session

---

### Audit Data (From Regis)

**Current surface area:**
- 26 files importing from `store.ts` (7 direct + 19 via `useAppContext`)
- 19 components consuming state
- ~56 `dispatch()` call sites
- ~100+ `state.` property accesses in TSX
- 15 test files touching store
- 9 test files with manual provider boilerplate

**Largest hotspot:** `useZoneConnection.ts` (577 lines, 15 handlers, highest value + highest risk)

**Key finding:** Re-render problem is measured — every context change triggers ALL consumers. Zustand's selectors fix this completely.
