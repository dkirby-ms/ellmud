...
- **Help Modal — Structured data over ANSI text (2026):** Built `HelpModal.tsx` as a modal overlay for the `help` command. Server returns structured `HelpDataMessage` (via new `HELP_DATA` MessageType) instead of pre-formatted ANSI text. Client renders modal with commands grouped by category. `CommandResult` gained optional `helpData` field; `deliverResult` in ZoneRoom sends it as separate message. Connection service and `useZoneConnection` hook extended with `onHelpData` handler + `helpData`/`clearHelpData` state. Modal follows WhoListModal pattern. `help <cmd>` scrolls to focused command. Key files: `HelpModal.tsx`, shared `index.ts`, server `help.ts`, `ZoneRoom.ts`, `connection.ts`, `useZoneConnection.ts`, `ZoneExploration.tsx`. Commit 6eb21b0.
...
- **Issue #443 — Minimap rendering bug with progressive exploration (2026-04-12):** Fixed rooms appearing in wrong positions as maps were revealed. Root cause: `sendExplorationData()` only sent the current room on join, not full exploration history. This meant client-side `computeLayout()` recalculated positions from scratch with incomplete data each time a new room was discovered, causing previously placed rooms to shift. Solution: Made `sendExplorationData()` async, loaded all previously visited rooms from `explorationRepo.getExploredRoomsInZone()`, and sent complete room array in EXPLORATION_DATA message. Layout engine now has stable foundation and maintains correct relative positions as new areas are revealed. Server-side fix only — no client changes needed. All 18 exploration tests pass. PR #444 (commit cd787d1).
