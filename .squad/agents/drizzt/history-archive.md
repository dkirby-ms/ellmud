...
- 2026-05-18T09:42:59.318-05:00 — Demo load generation should create real loopback WebSocket clients with the `ws` library instead of spoofing metrics so Azure Container Apps / KEDA sees genuine socket pressure.
- 2026-05-18T09:42:59.318-05:00 — `SIMULATE_LOAD=true` and `SIMULATE_LOAD=1` both map to the default 50-connection demo target; explicit numeric values become exact connection targets; `false`/unset disables auto-start.
- 2026-05-18T09:42:59.318-05:00 — Runtime control lives at `/admin/api/load-simulator/{status,start,stop}` behind `adminAuth`; the POST start payload uses `{ connections }`.
- 2026-05-18T09:42:59.318-05:00 — Key files: `packages/server/src/load-simulator/index.ts`, `packages/server/src/index.ts`, `packages/server/src/config.ts`, `packages/server/src/__tests__/load-simulator.test.ts`.

### 2026-05-18: Load Simulator Module (KEDA Load Testing)

**Status:** ✅ Complete — Module built and tests passing

**What was done:**
- Built `packages/server/src/load-simulator/` module with WebSocket client pool manager
- Admin API: `GET /admin/api/load-simulator/status`, `POST /admin/api/load-simulator/start`, `POST /admin/api/load-simulator/stop`
- Environment variable control: `SIMULATE_LOAD` env var sets target connection count
- Graceful ramp: 5 connections/second with periodic ping frames
- All tests passing (Minsc wrote comprehensive test suite)

**Key design decision:** Real WebSocket connections (not fake counters) for authentic autoscaling pressure observations in KEDA demos. Simulator kept in-process for straightforward startup, shutdown, and admin control.

**Integration:** Works with Khelben's infrastructure changes (SIMULATE_LOAD env var wiring in Bicep). See orchestration logs and decisions.md.
