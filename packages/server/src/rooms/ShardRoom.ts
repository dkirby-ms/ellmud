...
export class ShardRoom extends Room<ShardRoomOptions> {
  ...
  private handleCollapse(): void {
    ...
    this.broadcast(MessageTypes.NARRATE, {
      text: 'The dungeon shatters. Reality folds in on itself. Everything goes dark. A deep sickness settles into your bones — void-sickness consumes you.',
      type: 'system',
      timestamp: Date.now(),
  ...
  private handlePlayerDeath(playerId: string, playerName: string, roomId: string, killerIds?: string[]): void {
    ...
    if (client) {
      this.sendOverlayState(client, {
        ...
        state: 'death',
        narration: isPvPKill
          ? 'A rival adventurer fells you. You awaken in the Refuge, wracked with void-sickness…'
          : 'The darkness claims you. You awaken in the Refuge, weakened by void-sickness…',
        timestamp: Date.now(),
      });
