...
export class PgRunHistoryRepository implements RunHistoryRepository {
  async recordRun(run: RunRecord): Promise<void> {
    await query(
      `INSERT INTO run_history (run_id, player_id, shard_tier, biome, duration_sec, extracted, extracted_items, xp_gained)
  ...
  async getPlayerHistory(playerId: string, limit = 50): Promise<RunRecord[]> {
    const result = await query<RunRow>(
      `SELECT run_id, player_id, shard_tier, biome, duration_sec, extracted, extracted_items, xp_gained
       FROM run_history
       WHERE player_id = $1
    ...
    return result.rows.map((row) => ({
      ...
      biome: row.biome as BiomeType | null,
