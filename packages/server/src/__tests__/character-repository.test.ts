...
describe('InMemoryCharacterRepository', () => {
  ...
  describe('create', () => {
    it('creates a character and returns it', async () => {
      const char = await repo.create(PLAYER_A, 'Drizzt', 'veil');
      expect(char.name).toBe('Drizzt');
      expect(char.factionSlug).toBe('veil');
    ...
    it('rejects duplicate names for same player (case-insensitive)', async () => {
      await repo.create(PLAYER_A, 'Drizzt', 'veil');
      await expect(repo.create(PLAYER_A, 'drizzt', 'scarlet')).rejects.toThrow('already in use');
    });

    it('allows same name for different players', async () => {
      await repo.create(PLAYER_A, 'Drizzt', 'veil');
      const char = await repo.create(PLAYER_B, 'Drizzt', 'scarlet');
      expect(char.name).toBe('Drizzt');
    });
    ...
    it('allows reusing name after soft-delete', async () => {
      ...
      const newChar = await repo.create(PLAYER_A, 'Drizzt', 'scarlet');
  ...
  describe('list', () => {
    ...
    it('returns all non-deleted characters', async () => {
      await repo.create(PLAYER_A, 'Drizzt', 'veil');
      await repo.create(PLAYER_A, 'Bruenor', 'ironwright');
      const deleted = await repo.create(PLAYER_A, 'Wulfgar', 'scarlet');
    ...
    it('does not return characters from other players', async () => {
      await repo.create(PLAYER_A, 'Drizzt', 'veil');
      await repo.create(PLAYER_B, 'Bruenor', 'ironwright');
  ...
  describe('setActive', () => {
    it('activates a character and deactivates others', async () => {
      const char1 = await repo.create(PLAYER_A, 'Drizzt', 'veil');
      const char2 = await repo.create(PLAYER_A, 'Bruenor', 'ironwright');
    ...
    it('throws for character owned by different player', async () => {
      const char = await repo.create(PLAYER_B, 'Drizzt', 'veil');
