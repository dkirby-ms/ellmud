...
function playerProfileRepositoryContractTests(
  ...
  describe('save and load', () => {
    it('save() then load() returns the same data', async () => {
      ...
      await repo.save(PLAYER_A, profile);

      const loaded = await repo.load(PLAYER_A);
    ...
    it('load() for unknown player returns null', async () => {
      const loaded = await repo.load('ghost-player');
  ...
  describe('upsert (save overwrites existing)', () => {
    it('second save() overwrites skills', async () => {
      await repo.save(PLAYER_A, makeProfile({ skills: { stealth: 5, awareness: 5 } }));
      await repo.save(PLAYER_A, makeProfile({ skills: { stealth: 20, awareness: 15 } }));
    ...
    it('second save() overwrites maxCarryWeight', async () => {
      await repo.save(PLAYER_A, makeProfile({ maxCarryWeight: 20 }));
      await repo.save(PLAYER_A, makeProfile({ maxCarryWeight: 50 }));
    ...
    it('second save() overwrites equipment', async () => {
      await repo.save(PLAYER_A, makeProfile({
    ...
    it('second save() can remove equipment by saving undefined', async () => {
      ...
      await repo.save(PLAYER_A, makeProfile()); // no equipment
  ...
  describe('player isolation', () => {
    it('profiles are completely isolated between players', async () => {
      await repo.save(PLAYER_A, makeProfile({ skills: { stealth: 10, awareness: 5 } }));
      await repo.save(PLAYER_B, makeProfile({ skills: { stealth: 3, awareness: 20 } }));

      const loadedA = await repo.load(PLAYER_A);
      const loadedB = await repo.load(PLAYER_B);
    ...
    it('saving one player does not affect another', async () => {
      await repo.save(PLAYER_A, makeVeteranProfile());
      await repo.save(PLAYER_B, makeProfile());

      // Overwrite A, check B unaffected
      await repo.save(PLAYER_A, makeProfile({ skills: { stealth: 1, awareness: 1 } }));
    ...
    it('three players all have independent profiles', async () => {
      await repo.save(PLAYER_A, makeProfile({ maxCarryWeight: 10 }));
      await repo.save(PLAYER_B, makeProfile({ maxCarryWeight: 30 }));
      await repo.save(PLAYER_C, makeProfile({ maxCarryWeight: 50 }));

      expect((await repo.load(PLAYER_A))!.maxCarryWeight).toBe(10);
      expect((await repo.load(PLAYER_B))!.maxCarryWeight).toBe(30);
      expect((await repo.load(PLAYER_C))!.maxCarryWeight).toBe(50);
    });
  });
  ...
  describe('skill progression', () => {
    ...
    it('preserves carry weight upgrades', async () => {
      await repo.save(PLAYER_A, makeProfile({ maxCarryWeight: 20 }));
      await repo.save(PLAYER_A, makeProfile({ maxCarryWeight: 35 }));
      await repo.save(PLAYER_A, makeProfile({ maxCarryWeight: 50 }));
  ...
  describe('edge cases', () => {
    ...
    it('handles zero maxCarryWeight', async () => {
      await repo.save(PLAYER_A, makeProfile({ maxCarryWeight: 0 }));
    ...
    it('handles many players saved concurrently', async () => {
      ...
      const saves = Array.from({ length: profileCount }, (_, i) =>
        repo.save(`player-${i}`, makeProfile({ maxCarryWeight: i * 10 })),
      );

      await Promise.all(saves);

      const loaded0 = await repo.load('player-0');
      expect(loaded0!.maxCarryWeight).toBe(0);

      const loaded49 = await repo.load('player-49');
  ...
  describe('concurrent-like operations', () => {
    it('parallel saves for different players do not interfere', async () => {
      await Promise.all([
        repo.save(PLAYER_A, makeProfile({ skills: { stealth: 10, awareness: 5 } })),
        repo.save(PLAYER_B, makeProfile({ skills: { stealth: 3, awareness: 20 } })),
        repo.save(PLAYER_C, makeProfile({ skills: { stealth: 50, awareness: 50 } })),
      ]);

      expect((await repo.load(PLAYER_A))!.skills.stealth).toBe(10);
      expect((await repo.load(PLAYER_B))!.skills.awareness).toBe(20);
      expect((await repo.load(PLAYER_C))!.skills.stealth).toBe(50);
    });
    ...
    it('parallel saves and reads for different players', async () => {
      await repo.save(PLAYER_A, makeProfile({ maxCarryWeight: 100 }));
      await repo.save(PLAYER_B, makeProfile({ maxCarryWeight: 200 }));

      const [loadA, loadB] = await Promise.all([
        repo.load(PLAYER_A),
        repo.load(PLAYER_B),
        repo.save(PLAYER_C, makeProfile({ maxCarryWeight: 300 })),
      ]);

      expect(loadA!.maxCarryWeight).toBe(100);
      expect(loadB!.maxCarryWeight).toBe(200);

      const loadC = await repo.load(PLAYER_C);
  ...
  describe('full veteran profile round-trip', () => {
    it('saves and loads a fully populated profile', async () => {
      ...
      await repo.save(PLAYER_A, veteran);
